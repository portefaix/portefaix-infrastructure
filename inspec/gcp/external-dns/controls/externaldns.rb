...
sa_name = attribute("external_dns_sa_name")
k8s_ns = attribute("external_dns_k8s_ns")
k8s_sa = attribute("external_dns_k8s_sa")

control 'external_dns-1' do
  impact 1.0
  ...
  tag resource: "external_dns"
  tag effort: 0.2

  describe google_service_account(project: project_id, name: sa_name) do
    it { should exist }
    its('display_name') { should cmp 'External DNS' }
  end

  describe google_project_iam_binding(project: project_id, role: "roles/dns.admin") do
    it { should exist }
