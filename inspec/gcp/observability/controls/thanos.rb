...
sa_name = attribute("thanos_sa_name")
bucket_name = attribute("thanos_bucket_name")
bucket_location = attribute("thanos_bucket_location")
kms_location = attribute("thanos_kms_location")
key_ring_name = attribute("thanos_key_ring_name")
key_name = attribute("thanos_key_name")
k8s_ns = attribute("thanos_k8s_ns")
k8s_sa = attribute("thanos_k8s_sa")
...
control 'thanos-1' do
  ...
  describe google_service_account(project: project_id, name: sa_name) do
    it { should exist }
    its('display_name') { should cmp 'thanos' }
  end
