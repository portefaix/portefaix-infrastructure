...
sa_name = attribute("loki_sa_name")
bucket_name = attribute("loki_bucket_name")
bucket_location = attribute("loki_bucket_location")
kms_location = attribute("loki_kms_location")
key_ring_name = attribute("loki_key_ring_name")
key_name = attribute("loki_key_name")
k8s_ns = attribute("loki_k8s_ns")
k8s_sa = attribute("loki_k8s_sa")
...
control 'loki-1' do
  ...
  describe google_service_account(project: project_id, name: sa_name) do
    it { should exist }
    its('display_name') { should cmp 'loki' }
  end
