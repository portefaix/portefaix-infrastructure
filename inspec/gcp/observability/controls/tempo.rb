...
sa_name = attribute("tempo_sa_name")
bucket_name = attribute("tempo_bucket_name")
bucket_location = attribute("tempo_bucket_location")
kms_location = attribute("tempo_kms_location")
key_ring_name = attribute("tempo_key_ring_name")
key_name = attribute("tempo_key_name")
k8s_ns = attribute("tempo_k8s_ns")
k8s_sa = attribute("tempo_k8s_sa")
...
control 'tempo-1' do
  ...
  describe google_service_account(project: project_id, name: sa_name) do
    it { should exist }
    its('display_name') { should cmp 'tempo' }
  end
