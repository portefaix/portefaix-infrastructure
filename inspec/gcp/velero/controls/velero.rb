...
sa_name = attribute("velero_sa_name")
bucket_name = attribute("velero_bucket_name")
bucket_location = attribute("velero_bucket_location")
kms_location = attribute("velero_kms_location")
key_ring_name = attribute("velero_key_ring_name")
key_name = attribute("velero_key_name")
k8s_ns = attribute("velero_k8s_ns")
k8s_sa = attribute("velero_k8s_sa")
...
control 'velero-1' do
  ...
  describe google_service_account(project: project_id, name: sa_name) do
    it { should exist }
    its('display_name') { should cmp 'velero' }
  end
...
control 'velero-3' do
  ...
  # describe google_project_iam_custom_role(project: project_id, name: role_name) do
  #   it { should exist }
  #   # its('stage') { should eq 'GA' }
  #   its('included_permissions') { should eq [
  #     "compute.disks.get",
