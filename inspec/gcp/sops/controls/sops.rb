...
key_ring_name = attribute("key_ring_name")
key_name = attribute("key_name")
k8s_ns = attribute("k8s_ns")
k8s_sa = attribute("k8s_sa")
...
control 'sops-1' do
  ...
  describe google_service_account(project: project_id, name: sa_name) do
    it { should exist }
    its('display_name') { should cmp 'Sops' }
  end

  describe google_project_iam_binding(project: project_id, role: "roles/cloudkms.cryptoKeyEncrypterDecrypter") do
    it { should exist }
...
control 'sops-2' do
  ...
  describe google_kms_crypto_key(project: project_id, location: location, key_ring_name: key_ring_name, name: key_name) do
    it { should exist }
